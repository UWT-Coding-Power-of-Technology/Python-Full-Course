...
if HAS_MEDIAPIPE:
    try:
        ...
        # Lowered detection confidence to 0.5 for better detection on static images
...
def create_test_image():
    ...
    cv2.putText(img, "Upload an Image to Start", (150, 300),
...
def apply_filter(image, ftype):
    ...
    if HAS_MEDIAPIPE:
        ...
        if results.multi_hand_landmarks and results.multi_handedness:
            ...
            for i, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                cv2.line(img, tp, ip, (255, 0, 0), 2)

                # Distance check
                dist = np.sqrt((tp[0] - ip[0])**2 + (tp[1] - ip[1])**2)
                if dist < 40:  # Slightly increased threshold
                    hand_status = f"{label} Pinch"
                else:
                    hand_status = f"{label} Open"

                status_msg += f"{hand_status} | "
    ...
    cv2.putText(img, status_msg, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                (0, 255, 0), 2)
    return img
...
def index():
    ...
    <html>
    <head>
        <title>Real-Time Gesture & Media Control</title>
        <style>
    ...
    <body>
        <div class="container">
            <h1>🎨 Real-Time Gesture & Media Control</h1>
